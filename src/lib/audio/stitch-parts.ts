 ...
 *
 * The ordering is the whole point: `partPath` zero-pads the attempt id and the
 * chunk index so one lexicographic sort puts the sittings in the order they
 * happened and the chunks in order within each.
 */
export async function listParts(sessionId: string): Promise<PartFile[]> {
  return list(RAW_BUCKET, partsPrefix(sessionId));
}
  ...
  const workers = Array.from(
    ...
    async () => {
      for (let i = next++; i < parts.length; i = next++) {
        const stored = await download(RAW_BUCKET, `${prefix}/${parts[i].name}`);
        if (!stored) {
